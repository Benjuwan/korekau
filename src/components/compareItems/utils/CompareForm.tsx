    ...
    return (
        <form action="" className='shadow-[0_0_8px_rgba(160,160,160,.5)_inset] p-[1em] w-full rounded text-[0.875rem]' onSubmit={(formElm: ChangeEvent<HTMLFormElement>) => {
            ...
            <label htmlFor="amount" className="flex flex-row flex-wrap w-full align-center gap-[2%] mb-[1em] leading-loose">
                <span className="inline-block w-[30%] border-l-4 border-l-[#333] pl-[.5em]">容量・個数</span>
                <input type="text" inputMode="numeric" pattern="\d*" value={compareItem.amount} id="amount" className="text-[1rem] rounded w-[68%] border border-[#333] pl-[.5em]" min={0} onInput={(e: SyntheticEvent<HTMLInputElement>) => handleFormEntries<CompareItemsType>(e, compareItem, setCompareItem, "compare")} />
            </label>
            <label htmlFor="price" className="flex w-full align-center gap-[2%] mb-[1em] leading-loose">
                <span className="inline-block w-[30%] border-l-4 border-l-[#333] pl-[.5em]">価格</span>
                <input type="text" inputMode="numeric" pattern="\d*" value={compareItem.price} id="price" className="text-[1rem] rounded w-[68%] border border-[#333] pl-[.5em]" min={0} onInput={(e: SyntheticEvent<HTMLInputElement>) => handleFormEntries<CompareItemsType>(e, compareItem, setCompareItem, "compare")} />
            </label>
            <button className="text-white bg-[#333] rounded border border-transparent leading-11 w-full disabled:text-[#999] disabled:bg-[#dadada] not-disabled:hover:cursor-pointer not-disabled:hover:text-[#333] not-disabled:hover:border-[#333] not-disabled:hover:bg-white" disabled={compareItem.price.toString().length === 0 || compareItem.amount.toString().length === 0}>計算</button>
        </form>
    );
});